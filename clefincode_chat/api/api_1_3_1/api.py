    ...
    try:
        ...
        if channel_doc.type == "Guest":            
            ...
            if channel_doc.chat_profile.startswith("Guest"):
                # ✅ Keep this: needed for guest realtime updates
                ...
                for member in channel_doc.members:
                    if share_everyone == 0:
                        share_doctype("ClefinCode Chat Message", new_message.name, member.user)

                    results["room"] = room
                    results["send_date"] = convert_utc_to_user_timezone(
                        send_date,
                        get_user_timezone(member.user)["results"][0]["time_zone"]
                    )
                    results["time_zone"] = frappe.db.get_value("User", member.user, "time_zone")
                    results["target_user"] = member.user

                    # ✅ KEY FIX: skip sender to prevent double display
                    if member.user != email:
                        frappe.publish_realtime(
                            event=room,
                            message=results,
